...
function reloadBrowser(gulpMode:boolean = true, silent:boolean = true):void {
  ...
  if (gulpMode) {
    return gulpif(releaseMode, livereload({ silent: silent }), browserSync.reload());
  } else {
    if (releaseMode) {
...
gulp.task('watch', () => {
  ...
  gulp.watch(g$.sourceFiles.es, ['loadConfig', 'js', 'jsLint']);
  gulp.watch(g$.sourceFiles.jade, ['loadConfig', 'jade']);
  gulp.watch(g$.sourceFiles.jsHead, ['loadConfig', 'jsHead']);
  gulp.watch([
  ...
  gulp.watch(g$.sourceFiles.html, ['loadConfig', 'html']);
  gulp.watch(g$.sourceFiles.partials, ['loadConfig', 'partials']);
  gulp.watch(g$.sourceFiles.fonts, ['loadConfig', 'fonts']);
  gulp.watch(g$.sourceFiles.vendor.js, g$.sourceFiles.vendor.css, ['loadConfig', 'vendorJs']);
  gulp.watch(g$.sourceFiles.handlebars, ['loadConfig', 'handlebars']);

  /*gulp.watch(g$.sourceFiles.html).on('change', browserSync.reload);
  gulp.watch(g$.sourceFiles.js).on('change', browserSync.reload);*/
});
...
gulp.task('loadConfig', () => {
  ...
  return gutil.log(`\n**************************\nGenerating ${g$.environment} Build\n**************************`);
});
...
gulp.task('babel', () => {
  ...
  g$.deleteFiles([g$.source + '_compiled']);
